...
if (isset($_POST["order_id"])) {
    ...
    $orderContent = '
        ...
        <p>Address: <span style="color: brown;">'.$row["customer_address"].'</span></p>
        <p>Order Items:</p>';

    // Decode the JSON string to an array
    $order_items = json_decode($row["order_item"], true);

    // Loop through the order items and add them to the PDF
    foreach ($order_items as $item) {
        $item_name = $item['name'];
        $item_price = $item['price'];
        $item_quantity = $item['quantity'];

        $orderContent .= '<p>'.$item_name.' x '.$item_quantity.'</p>';
    }

    $orderContent .= '
        <p>Total Price: <span style="color: brown;">RM '.$row["order_total_price"].'</span></p>
